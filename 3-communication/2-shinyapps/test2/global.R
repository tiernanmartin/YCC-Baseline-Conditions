...
linkedBarMap <-
                  ...
                  plotly_event_rx)
        {
                ...
                observe({
                        ...
                        switch_labFrmt <- {
                                if(miscgis::is_pct(sp_rx_id()[[y_axis()]])){
                                        function(type,...)labelFormat(suffix = '%',transform = function(x) round_any(100 * x, .1))
                                }else{
                                        function(type,...)labelFormat()
                                }
                                }
                ...
                output$bar <- renderPlotly({
                        pal <- colorpal()
                        gg1 <- {ggplot(sp_rx_id()@data) + geom_bar(
                                        aes(
                                                x = sp_rx_id()[[x_axis()]],
                                                y = sp_rx_id()[[y_axis()]],
                                                colour = sp_rx_id()[[y_axis()]],
                                                fill = sp_rx_id()[[y_axis()]],
                                                key = KEY
                                        ),
                                        stat = "identity",
                                        alpha = .85
                        ...
                        gg2 <- {
                                if(is.numeric(sp_rx()[[y_axis()]])){
                                        gg1 +
                                                scale_color_gradientn(colors = pal_choice_rev()) +
                                                scale_fill_gradientn(colors = pal_choice_rev())
                                }else{
                                        gg1 +
                                                scale_color_manual(values = pal(sp_rx_id()[[y_axis()]])) +
                                                scale_fill_manual(values = pal(sp_rx_id()[[y_axis()]]))
                        ...
                        gg4 <- {
                                y <- {if(miscgis::is_pct(sp_rx_id()[[y_axis()]])){
                                        gg3 + 
                                                scale_y_continuous(labels = scales::percent)
                                }else if(is.numeric(sp_rx_id()[[y_axis()]]) & 
                                             min(sp_rx_id()[[y_axis()]],na.rm = TRUE)>1){
                                        gg3 +
                                                scale_y_continuous(labels = scales::comma)
                                }else gg3}
                                
                                xy <- {if(miscgis::is_pct(sp_rx_id()[[x_axis()]])){
                                        y + 
                                                scale_x_continuous(labels = scales::percent)
                                }else if(is.numeric(sp_rx_id()[[x_axis()]]) & 
                                         min(sp_rx_id()[[x_axis()]],na.rm = TRUE)>1){
                                        gg3 +
