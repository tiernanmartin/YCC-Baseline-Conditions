...
Upper-right is the `nc_grp`, which has simply been been transformed by `dyplr::mutate` (the lack of CRS is visually apparent).

Lower-left is `nc_grp` colored by the group (`nc_grp$GRP`)

Lower-right is `nc_grp` overlayed by the `summarise()` version: `nc_aggr` (notice that the white border lines do not align with the fill colors)
