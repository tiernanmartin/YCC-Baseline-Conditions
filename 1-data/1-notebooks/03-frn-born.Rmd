...
```{r frn-born-setup, echo = FALSE, warning=FALSE,message=FALSE,comment=FALSE}
library(plyr)
library(knitr)
library(rprojroot)
library(rgdal)
...
if(!file.exists(root_file('1-data/4-interim/frn-born-orig-acs.rds'))){
        
        tbl <- "B05002"  # census table code
        
        acs.fetch(endyear = 2014,
                  geography = ycc_geo_acs,
                  table.number = tbl) %>%
                write_rds(root_file('1-data/4-interim/frn-born-orig-acs.rds'))
        
}

frn_born_orig_acs <- read_rds(root_file('1-data/4-interim/frn-born-orig-acs.rds'))


# Process and save the data
# -----------------------------------------------------------------------------

if(!file.exists(root_file('1-data/4-interim/frn-born-acs.rds'))){
        frn_born_acs1 <- frn_born_orig_acs
        
        guide <- 
                data.frame(terse = acs.colnames(frn_born_acs1),
        ...
        frn_born <- get_col('Foreign\\sborn:$')
        
        
        # 1. Calculate the percentage of foreign born individuals: `FRN_BORN / TOTAL`
        
        acs_col_pct_frn_born <-  
                acs::apply(X = frn_born_acs1[,frn_born],
                           MARGIN = 1,
                           FUN = divide.acs,
                           denominator = frn_born_acs1[,total],
                           method = "proportion",
                           verbose = FALSE)
        acs.colnames(acs_col_pct_frn_born) <- 'PCT_FRN_BORN'
        
# Save the object
# -----------------------------------------------------------------------------
        
        acs_col_pct_frn_born %>% write_rds(root_file('1-data/4-interim/frn-born-acs.rds'))
        
}

frn_born_acs <- read_rds(root_file('1-data/4-interim/frn-born-acs.rds'))


# Plot the data
# -----------------------------------------------------------------------------

plot(frn_born_acs[order(estimate(frn_born_acs)),1])
...
ycc_frn_born_sf <- 
        data.frame(
                geography(frn_born_acs)["tract"],
                estimate(frn_born_acs), 
                1.645 * standard.error(frn_born_acs)) %>% 
        `colnames<-`(., c("JOIN_ID", "FRN_BORN_PCT_EST","FRN_BORN_PCT_MOE")) %>% 
        mutate(JOIN_ID = if_else(is.na(JOIN_ID),'SEA',JOIN_ID),
               UPPER = FRN_BORN_PCT_EST + FRN_BORN_PCT_MOE, 
               LOWER = FRN_BORN_PCT_EST - FRN_BORN_PCT_MOE, 
               FRN_BORN_PCT_UPPER = if_else(UPPER > 1, 1, UPPER), 
               FRN_BORN_PCT_LOWER = if_else(LOWER < 0, 0, LOWER)) %>% 
...
# Save the object
# -----------------------------------------------------------------------------
ycc_frn_born_sf %>% write_rds(root_file('1-data/4-interim/frn-born-sf.rds'))
...
```{r dotplot-frn-born-sf}

# Show dotplot with variance
# -----------------------------------------------------------------------------

view_frn_born_dotplot <- function() {
        
        df <- ycc_frn_born_sf %>% select(-geom) %>% as.data.frame()
        
        gg <- ggplot(df, aes(x = reorder(JOIN_ID, 
                                             FRN_BORN_PCT_EST), y = FRN_BORN_PCT_EST))
        gg <- gg + geom_linerange(aes(ymin = FRN_BORN_PCT_LOWER, 
                                      ymax = FRN_BORN_PCT_UPPER), size = 1.5, alpha = 0.25)
        ...
        gg <- gg + labs(x = NULL, y = NULL, title = "Foreign-Born Population (%)", 
                        subtitle = "Census Tracts", 
                        caption = "Source: ACS Table B25106, Five-Year Estimates (2010-2014)")
        gg
}

view_frn_born_dotplot()

# view_frn_born_dotplot_int <- function() {
#         ggiraph(code = {
#                 print(view_frn_born_dotplot())
...
pal <- colorNumeric(viridis(256, option = "C"),tr$FRN_BORN_PCT_EST)

myLfltGrey() %>% 
        addPolygons(data = as(tr,'Spatial'),
                fillOpacity = .75,
                fillColor = ~pal(FRN_BORN_PCT_EST),
                color = col2hex('white'),
                opacity = 1,
                weight = .5,
                smoothFactor = 0) %>% 
        addLegend(title = "Foreign-Born Population",
                  position = 'topright',
                  pal = pal, 
                  values = tr$FRN_BORN_PCT_EST,
                  opacity = .75,
