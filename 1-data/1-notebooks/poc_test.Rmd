...
# Tracts only
if(!file.exists(root_file('1-data/4-interim/poc-all-df.rds'))){
        ...
        write_rds(root_file('1-data/4-interim/poc-all-df.rds'))
}

poc_all_df <- read_rds(root_file('1-data/4-interim/poc-all-df.rds'))

# View the df object
# -----------------------------------------------------------------------------

view_poc_dotplot <- function() {
        gg <- ggplot(poc_all_df, aes(x = reorder(NAME, 
...
poc_sea_tr_df <- poc_all_df %>% filter(nchar(NAME) == 6)

# View the df object
# -----------------------------------------------------------------------------

view_poc_dotplot <- function() {
        gg <- ggplot(poc_sea_tr_df, aes(x = reorder(NAME, 
                                             POC_PCT_EST), y = POC_PCT_EST))
        gg <- gg + geom_linerange(aes(ymin = LOWER, 
                                      ymax = UPPER), size = 1.5, alpha = 0.25)
        gg <- gg + geom_point_interactive(aes(tooltip = NAME), 
...
if(!file.exists(root_file('1-data/4-interim/poc-sea-tr-sp.gpkg'))){
        
poc_sea_tr_sp <- tr_sea_sp[,'GEOID6']

poc_sea_tr_sp@data %<>% left_join(poc_sea_tr_df, by = c("GEOID6" = "NAME"))

poc_sea_tr_sp %>% writeOGR(dsn = root_file('1-data/4-interim/poc-sea-tr-sp.gpkg'),
                    layer = "poc_sp", driver = "GPKG", verbose = FALSE, 
         overwrite_layer = TRUE)
}

poc_sea_tr_sp <-  readOGR(dsn = root_file('1-data/4-interim/poc-sea-tr-sp.gpkg'),
                    layer = "poc_sp", 
                    verbose = FALSE, 
...
poc_ycc_df <- poc_all_df %>% filter(nchar(NAME) != 6)
