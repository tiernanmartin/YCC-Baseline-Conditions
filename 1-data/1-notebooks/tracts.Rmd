...
if(!file.exists(root_file('1-data/4-interim/nhood-ycc-sp.gpkg'))){
        df <- tr_ycc_sp@data[!duplicated(tr_ycc_sp$NHOOD_ABBR),] %>% 
        arrange(NHOOD_FCT)
rownames(df) <- df$NHOOD_ABBR
df %<>% select(-NHOOD_FCT)

nhood_ycc_sp <- SpatialPolygonsDataFrame(Sr = gUnaryUnion(tr_ycc_sp,id = tr_ycc_sp$NHOOD_ABBR),
                                 data = df,
                                 match.ID = TRUE)

nhood_ycc_sp %>% writeOGR(dsn = root_file('1-data/4-interim/nhood-ycc-sp.gpkg'),
                          layer = 'nhood_ycc_sp',
                          driver = 'GPKG',
                          verbose = FALSE)
}

nhood_ycc_sp <-  readOGR(dsn = root_file('1-data/4-interim/nhood-ycc-sp.gpkg'),
                          layer = 'nhood_ycc_sp',
                          verbose = FALSE,stringsAsFactors = FALSE) %>% spTransform(crs_proj)

# Map the sp object
# -----------------------------------------------------------------------------

nhood_ycc_sp@data %<>% mutate(NHOOD_FCT = factor(NHOOD,levels = c('Capitol Hill',
                                                               'Central Area',
                                                               'First Hill',
                                                               'Chinatown International District'),
                                              ordered = TRUE))
...
myLfltGrey() %>% 
        addPolygons(data = nhood_ycc_sp, 
                    ...
                    weight = .5,
                    smoothFactor = 0) %>% 
        addLegend(position = 'topright',pal = pal, values = nhood_ycc_sp$NHOOD_FCT,opacity = .85)
